...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function applyKMeans(data, clusters = 3, iterations = 10) {
    try {
        await tf.ready();
        
        if (!data || data.length === 0) {
            throw new Error("No hay datos para clasificar");
        }

        // Convertir a tensor 2D
        const tensorData = tf.tensor2d(data.map(v => [v]));
        
        // Inicializar centroides con valores reales del dataset
        const centroids = [];
        const step = Math.floor(data.length / clusters);
        for (let i = 0; i < clusters; i++) {
            centroids.push([data[Math.min(i * step, data.length - 1)]]);
        }
        let currentCentroids = tf.tensor2d(centroids);

        for (let i = 0; i < iterations; i++) {
            // Calcular distancias
            const expandedData = tensorData.expandDims(1);
            const expandedCentroids = currentCentroids.expandDims(0);
            const distances = tf.sqrt(
                tf.sum(
                    tf.square(
                        tf.sub(expandedData, expandedCentroids)
                    ),
                    2
                )
            );
            ...
            const assignments = await tf.argMin(distances, 1).data();

            // Recalcular centroides (versión sin booleanMask)
            const newCentroids = [];
            for (let c = 0; c < clusters; c++) {
                ...
                let newCentroid;
                if (indices.length > 0) {
                    const groupedData = indices.map(idx => data[idx]);
                    const mean = groupedData.reduce((sum, val) => sum + val, 0) / groupedData.length;
                    newCentroid = [mean];
                } else {
                    // Si no hay puntos en el cluster, mantener el centroide anterior
                    newCentroid = currentCentroids.arraySync()[c];
                }
                newCentroids.push(newCentroid);
            }
            
            // Actualizar centroides
            const oldCentroids = currentCentroids;
            currentCentroids = tf.tensor2d(newCentroids);
            oldCentroids.dispose();
        }

        // Calcular asignaciones finales
        const finalExpandedData = tensorData.expandDims(1);
        const finalExpandedCentroids = currentCentroids.expandDims(0);
        const finalDistances = tf.sqrt(
            tf.sum(
                tf.square(
                    tf.sub(finalExpandedData, finalExpandedCentroids)
                ),
                2
            )
        );
        
        const finalAssignments = await tf.argMin(finalDistances, 1).data();

        // Liberar memoria
        tensorData.dispose();
        currentCentroids.dispose();

        return Array.from(finalAssignments).map(g => `Grupo ${g + 1}`);
    } catch (error) {
        console.error("Error en applyKMeans:", error);
        throw error;
    }
}
});
